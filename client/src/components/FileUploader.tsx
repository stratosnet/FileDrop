...
export default function FileUploader({ 
  ...
  const handleFileUpload = useCallback(async (file: File) => {
        ...
        prevUploads.map(upload => 
          upload.id === uploadId 
            ? { 
                ...upload, 
                status: 'error', 
                errorMessage: `File too large. Maximum size is ${formatFileSize(maxFileSize)}.` 
              } 
            : upload
        )
      );
    ...
    setUploads(prevUploads => 
      prevUploads.map(upload => 
        upload.id === uploadId ? { ...upload, status: 'uploading', progress: 0 } : upload
      )
    );
    ...
    try {
      ...
      xhr.upload.addEventListener('progress', (event) => {
        if (event.lengthComputable) {
          ...
          setUploads(prevUploads => 
            prevUploads.map(upload => 
              upload.id === uploadId ? { ...upload, progress: cappedProgress } : upload
            )
          );
        }
      });
      ...
      xhr.addEventListener('load', () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          handleUploadError(uploadId, `Server error: ${xhr.status} ${xhr.statusText}`);
        }
      });
      ...
      handleUploadError(uploadId, error instanceof Error ? error.message : 'Unknown error occurred');
    }
  ...
  return (
    <main>
      {/* File Drop Zone */}
      <div 
        className={`file-drop-zone rounded-lg p-10 text-center mb-8 cursor-pointer bg-white shadow-sm hover:shadow-md transition-all ${isDragging ? 'active border-blue-500 bg-blue-50' : 'border-2 border-dashed border-gray-300'}`}
      ...
      >
        ...
        <h2 className="text-xl font-semibold mb-2">Drop files here or click to browse</h2>
        <p className="text-gray-500 mb-4">Upload any file type. No login required.</p>
        <input 
          type="file" 
          id="fileInput" 
          className="hidden" 
